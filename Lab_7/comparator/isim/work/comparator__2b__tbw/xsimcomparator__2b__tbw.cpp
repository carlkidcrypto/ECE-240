...
va_list vap)
{
    extern HSim__s6 * createworkMcomparator__2b__tbw(const char*);
    HSim__s6 *blk = createworkMcomparator__2b__tbw(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_comparator_2b_behavioral(const char*);
    HSim__s6 *blk = createWork_comparator_2b_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","comparator_2b_tbw", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("comparator_2b/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("comparator_2b", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMcomparator__2b__tbw(const char*);
        topvl = (HSim__s5*)createworkMcomparator__2b__tbw("comparator_2b_tbw");
